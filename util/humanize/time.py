    ...
    if not years and days < 1:
        if seconds == 0:
            if minimum_unit == Unit.MICROSECONDS and delta.microseconds < 1000:
                return (
                    ngettext("%d milisegundo", "%d microsegundo", delta.microseconds)
                    % delta.microseconds
                )
                ...
                return (
                    ngettext("%d milisegundo", "%d microsegundo", milliseconds)
                    % milliseconds
                )
            return _("un momento")
        elif seconds == 1:
            ...
            return ngettext("%d segundo", "%d segundos", seconds) % seconds
        elif 60 <= seconds < 120:
            return _("1 minuto")
        elif 120 <= seconds < 3600:
            minutes = seconds // 60
            return ngettext("%d minuto", "%d minutos", minutes) % minutes
        elif 3600 <= seconds < 3600 * 2:
            return _("1 hora")
        elif 3600 < seconds:
            hours = seconds // 3600
            return ngettext("%d hora", "%d horas", hours) % hours
    elif years == 0:
        if days == 1:
            return _("1 día")
        if not use_months:
            return ngettext("%d día", "%d días", days) % days
        else:
            if not months:
                return ngettext("%d día", "%d días", days) % days
            elif months == 1:
                ...
                return ngettext("%d mes", "%d meses", months) % months
    elif years == 1:
        if not months and not days:
            return _("a año")
        elif not months:
            return ngettext("1 año, %d día", "1 año, %d días", days) % days
        elif use_months:
            if months == 1:
                return _("1 año, 1 mes")
            else:
                return (
                    ngettext("1 año, %d mes", "1 año, %d meses", months) % months
                )
        else:
            return ngettext("1 año, %d día", "1 año, %d días", days) % days
    else:
        return ngettext("%d año", "%d años", years) % years
...
def precisedelta(value, minimum_unit="seconds", suppress=(), format="%0.2f"):
    ...
    fmts = [
        ("%d año", "%d años", years),
        ("%d mes", "%d meses", months),
        ("%d día", "%d días", days),
        ("%d hora", "%d horas", hours),
        ("%d minuto", "%d minutos", minutes),
        ("%d segundo", "%d segundos", secs),
        ("%d millisegundo", "%d millisegundos", msecs),
        ("%d microsegundo", "%d microsegundos", usecs),
    ]
